...
#efficiency <- log(actual_daily$sum / execution_daily$sum)
...
##### TQuest method (Feature-based distances)
```{r}
dtw_function <- function(x,begin_date,end_date){
  x$date <-as.Date(x$date)
  x <- x[x$date >=begin_date & x$date<= end_date,]
...
ggplot(plot, aes(x=V1,y = V2)) +
    ...
    geom_line(aes(x = V1, y = V3), color = 'blue') 
...
d <- efficiency_daily %>% 
  filter(NEWID==300) %>%
  filter(date>= "2021-03-29" & date <="2021-07-04")
plot <- as.data.frame(cbind(c(1:98),a$sum,b$sum,c$sum,d$sum))
ggplot(plot, aes(x=V1,y = V2)) +
    geom_line(aes(x = V1, y = V2), color = 'red') +
    geom_line(aes(x = V1, y = V3), color = 'green') +
    geom_line(aes(x = V1, y = V4), color = 'blue') +
    geom_line(aes(x = V1, y = V5), color = 'purple')
```
